...
md_content = """@title Combinatorics & Packing — Quantitative Assessment
...
@explanation Each ball has diameter $d = 2r = 6\ \mathrm{cm}$. The balls are arranged in 2 rows and 4 columns, so width = $2d = 12\ \mathrm{cm}$, length = $4d = 24\ \mathrm{cm}$, and height = $d = 6\ \mathrm{cm}$. Dimensions (height × width × length) are $6 \times 12 \times 24$, so option (D) is correct.
@subject Quantitative
...
out = Path(out_dir / "Math_Question_Generation_Output.md")
